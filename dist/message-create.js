...
const defaultMessageCreator = (key, area) => `${key} - ${area.areaName}`;
export default function mapCommand(config) {
    return async (message) => {
        ...
        for (const key of Object.keys(config)) {
            ...
            const content = (element.messageCreator ?? defaultMessageCreator)(key, areaInfo, room);
            if (textOnly) {
                try {
            ...
            try {
                ...
                const bytes = renderer.export(new PngBytesExporter({
                    width: 1200,
                    height: 1200,
